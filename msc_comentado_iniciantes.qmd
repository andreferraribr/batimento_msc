...
relnr <- read_fwf("RELNR.TXT", 
                  ...
                  col_positions = fwf_positions(
                      start = c(1, 12, 39, 43, 53, 80),    # posição inicial de cada coluna
...
relnd <- read_fwf("RELND.TXT", 
                  ...
                  col_positions = fwf_positions(
                      ...
                      col_names = c("ND_SICONFI", "TITULO_SICONFI", "SITUACAO_SICONFI", 
                                    "ND_UNIAO", "TITULO_UNIAO", "SITUACAO_UNIAO")
                  ),
...
# Análise Detalhada (Para Especialistas)
...
# Armazenar saldos finais para comparação
saldos_msc <- saldos_resumo_msc
```
...
cat("=== FONTES DE DADOS CARREGADAS ===\n")
cat("- Balancete SIAFI:", nrow(balancete_siafi), "registros\n")
cat("- Balancete TG MSC:", nrow(balancete_tg_msc), "registros\n")
cat("- Saldos MSC consolidados:", nrow(saldos_msc), "classes\n\n")
```
...
cat("=== RESUMO EXECUTIVO DA CONCILIAÇÃO ===\n")
cat("Classes aprovadas na conciliação:", sum(comparacao_saldos$conciliacao_ok), "de", nrow(comparacao_saldos), "\n")
cat("Classes com divergências:", sum(!comparacao_saldos$conciliacao_ok), "de", nrow(comparacao_saldos), "\n")
cat("Taxa de aprovação:", round(mean(comparacao_saldos$conciliacao_ok) * 100, 1), "%\n\n")
...
# Estatísticas de qualidade das diferenças
cat("=== MÉTRICAS DE QUALIDADE DOS DADOS ===\n")
cat("Diferenças MSC vs SIAFI:\n")
cat("  Máxima: R$", formatC(max(abs(comparacao_saldos$diff_msc_siafi)), format = "f", digits = 2, big.mark = "."), "\n")
cat("  Média: R$", formatC(mean(abs(comparacao_saldos$diff_msc_siafi)), format = "f", digits = 2, big.mark = "."), "\n")

cat("Diferenças MSC vs TG_MSC:\n")
cat("  Máxima: R$", formatC(max(abs(comparacao_saldos$diff_msc_tg)), format = "f", digits = 2, big.mark = "."), "\n")
cat("  Média: R$", formatC(mean(abs(comparacao_saldos$diff_msc_tg)), format = "f", digits = 2, big.mark = "."), "\n")

cat("Diferenças SIAFI vs TG_MSC:\n")
cat("  Máxima: R$", formatC(max(abs(comparacao_saldos$diff_siafi_tg)), format = "f", digits = 2, big.mark = "."), "\n")
cat("  Média: R$", formatC(mean(abs(comparacao_saldos$diff_siafi_tg)), format = "f", digits = 2, big.mark = "."), "\n\n")
...
datatable(tabelao %>% group_by(classe, tipo_valor)%>% summarise(saldo = sum(saldo, na.rm = TRUE)) %>% pivot_wider(names_from = tipo_valor, values_from = saldo)%>% mutate(resultado = beginning_balance + period_change - ending_balance)  )%>% formatRound(c("beginning_balance","period_change", "ending_balance", "resultado"), 2, mark = ".", dec.mark = "," )

saldos_msc <- tabelao %>% group_by(classe, tipo_valor)%>% summarise(saldo = sum(saldo, na.rm = TRUE)) %>% pivot_wider(names_from = tipo_valor, values_from = saldo)%>% mutate(resultado = beginning_balance + period_change - ending_balance)  
...
balancete_siafi <-  read_excel("balancete_siafi.xlsx", 
    range = "A1:D9") %>% clean_names()

balancete_siafi <- balancete_siafi %>% mutate(classe = str_sub(conta, 1L, 1L))




balancete_tg_msc <- read_excel("balancete_tg_msc.xlsx") %>% clean_names() %>% rename(classe = 1) %>% filter(mes_lancamento == mes) %>% select(-mes_lancamento)
...
```{r}
# Criar dataframe comparativo dos saldos por classe
comparacao_saldos <- data.frame(
  classe = 1:8,
  saldo_msc = saldos_msc$ending_balance,
  saldo_siafi = balancete_siafi$saldo_atual,
...
# Calcular diferenças entre os saldos
comparacao_saldos <- comparacao_saldos %>%
  mutate(
    # Diferença entre saldo_msc e saldo_siafi
    diff_msc_siafi = saldo_msc - saldo_siafi,
    # Diferença entre saldo_msc e saldo_tg_msc
    diff_msc_tg = saldo_msc - saldo_tg_msc,
    # Diferença entre saldo_siafi e saldo_tg_msc
    diff_siafi_tg = saldo_siafi - saldo_tg_msc,
    
    # Verificar se as diferenças são inferiores a R$ 20,00
    diff_msc_siafi_ok = abs(diff_msc_siafi) < 20,
    diff_msc_tg_ok = abs(diff_msc_tg) < 20,
    diff_siafi_tg_ok = abs(diff_siafi_tg) < 20,
    
    # Status geral da conciliação por classe
    conciliacao_ok = diff_msc_siafi_ok & diff_msc_tg_ok & diff_siafi_tg_ok
  )

# Exibir o dataframe comparativo
print("=== COMPARAÇÃO DE SALDOS POR CLASSE ===")
print(comparacao_saldos)

# Resumo da conciliação
cat("\n=== RESUMO DA CONCILIAÇÃO ===\n")
cat("Classes com conciliação OK:", sum(comparacao_saldos$conciliacao_ok), "de", nrow(comparacao_saldos), "\n")
cat("Classes com divergências:", sum(!comparacao_saldos$conciliacao_ok), "de", nrow(comparacao_saldos), "\n")

# Identificar classes com problemas
classes_problemas <- comparacao_saldos[!comparacao_saldos$conciliacao_ok, ]
if(nrow(classes_problemas) > 0) {
  cat("\n=== CLASSES COM DIVERGÊNCIAS ===\n")
  print(classes_problemas[, c("classe", "diff_msc_siafi", "diff_msc_tg", "diff_siafi_tg")])
}

# Estatísticas das diferenças
cat("\n=== ESTATÍSTICAS DAS DIFERENÇAS ===\n")
cat("Diferença MSC vs SIAFI:\n")
cat("  Máxima:", max(abs(comparacao_saldos$diff_msc_siafi)), "\n")
cat("  Média:", mean(abs(comparacao_saldos$diff_msc_siafi)), "\n")

cat("\nDiferença MSC vs TG_MSC:\n")
cat("  Máxima:", max(abs(comparacao_saldos$diff_msc_tg)), "\n")
cat("  Média:", mean(abs(comparacao_saldos$diff_msc_tg)), "\n")

cat("\nDiferença SIAFI vs TG_MSC:\n")
cat("  Máxima:", max(abs(comparacao_saldos$diff_siafi_tg)), "\n")
cat("  Média:", mean(abs(comparacao_saldos$diff_siafi_tg)), "\n")
