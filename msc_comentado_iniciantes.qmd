...
portaria_PO <- read_excel("portaria.xlsx", sheet = "PO", skip = 4) %>% 
 ...
 # Renomeia a primeira coluna para um nome mais claro
  ...
  filter(str_detect(po_siconfi, ".*[0-9].*")) %>% 
  mutate(po_siconfi = as.character(po_siconfi))
...
preparar_codigos_validos <- function() {
  ...
  # PO: Complementos - manter todos 
  po_validos <- portaria_PO$po_siconfi %>% unique()
...
# FUNÇÃO 1 CORRIGIDA: Agrupa códigos por tipo
agrupar_tipos <- function(tipo) {
  # Busca o tipo em cada posição (1 a 6) e coleta os códigos únicos
  grupo_1 <- tabelao %>% filter(tipo1.msc == tipo, !is.na(ic1.msc)) %>% pull(ic1.msc) %>% unique()  # 👈 ADICIONADO!
  ...
  if(length(grupo_1) == 0 && length(grupo_2) == 0 && length(grupo_3) == 0 && 
     length(grupo_4) == 0 && length(grupo_5) == 0 && length(grupo_6) == 0) {
    return(data.frame(
      codigo = character(0),
      origem = character(0),
  ...
  # Combina todos os grupos em uma tabela (INCLUINDO grupo_1)
  data.frame(
    codigo = c(grupo_1, grupo_2, grupo_3, grupo_4, grupo_5, grupo_6),  # 👈 INCLUÍDO grupo_1!
    origem = c(rep("ic1", length(grupo_1)),  # 👈 ADICIONADO!
               rep("ic2", length(grupo_2)),
...
# PO_msc agora deve encontrar todos os 73 códigos únicos de PO
# A validação deve mostrar: PO = 73 códigos, 100% válidos ✅
...
executar_validacao_automatica <- function(dados) {
  ...
  dados_validados <- dados %>%
    mutate(
      # POSIÇÃO 1: Somente PO (Poder Órgão)
      posicao_1 = case_when(
        tipo1.msc == "PO" ~ ifelse(ic1.msc %in% codigos_validos$po_validos, "ok", "verificar"),
        TRUE ~ "demais"
      ),
      
      # POSIÇÃO 2: FP (1 ou 2) ou FR (validar com códigos filtrados)
      posicao_2 = case_when(
...
verificar_todos_codigos_correto <- function() {
  ...
  tribble(
    ...
    "PO", PO_msc, codigos_validos$po_validos,
  ...
  mutate(
    ...
    diagnostico = map2(dados, referencia, ~{
      ...
      tibble(
        ...
        status = if_else((encontrados_count / total_count) >= 1, "✅", "⚠️")
      )
    })
  ) %>%
...
PO_msc <- agrupar_tipos("PO")   # Poder Órgão
...
resumo_problemas <- tabelao_validado %>% 
  ...
  mutate(
    ...
    `% Total` = round(Problemas / nrow(tabelao_validado) * 100, 4)
  )
...
taxa_sucesso <- round((total_registros - registros_com_problema) / total_registros * 100, 4)
...
status_sistema <- data.frame(
  ...
  Status = c("📊", "✅", "⚠️", if(taxa_sucesso >= 100) "🏆" else "📈")
)
...
datatable(saldos_msc, 
          ...
          colnames = c("Classe", "Saldo Inicial", "Saldo Final", "Movimentação", "Resultado")) %>%
...
```{r}
 # datatable(tabelao_validado %>% filter(conta %in% c("522130100", "622110000","822210100", "822110100"), tipo2.msc == "FR" ) %>% group_by(conta, ic2.msc, tipo2.msc ) %>% count())
```

```{r verificar_campos_sem_registor}
# datatable(tabelao_validado %>% filter(!is.na(ic1.layout), is.na(ic1.msc))  %>% group_by(conta ) %>% count())%>%  formatRound("conta", digits = 0, dec.mark = "", mark = "")
```

```{r}
 # datatable(tabelao_validado %>% filter(conta %in% c("522130100", "622110000","822210100", "822110100"), tipo3.msc == "FR" ) %>% group_by(conta,  ic3.msc, tipo3.msc) %>% count()) 
```
